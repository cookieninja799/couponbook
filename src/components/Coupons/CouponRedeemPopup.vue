      ...
      <section v-else>
        ...
        <div v-else class="redeemed">
          ...
          <div class="coupon-detail">
            ...
            <img v-if="coupon.qrCodeUrl" :src="coupon.qrCodeUrl" alt="Coupon QR" class="qr" />
          </div>
  
          <div class="actions">
            <button class="btn" @click="printCoupon">Print</button>
            <a v-if="coupon.pdfUrl" class="btn" :href="downloadUrlSecured" target="_self" download>
              Download PDF
            </a>
    ...
    methods: {
      async confirmRedeem() {
        ...
        try {
          ...
          if (payload.download_token && this.coupon?.pdfUrl) {
            const u = new URL(this.coupon.pdfUrl, window.location.origin);
            u.searchParams.set('token', payload.download_token);
            this.coupon = { ...this.coupon, pdfUrl: u.toString() };
          }
