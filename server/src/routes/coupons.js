...
router.post('/:id/redeem', auth(), async (req, res, next) => {
  try {
    ...
    // 3) Map Cognito subject -> local user row via *cognitoSub* (NOT providerSub)
    ...
    if (!u) {
      ...
      // name is non-null in your schema; derive a safe fallback
      const safeName =
        req.user?.name ||
        (req.user?.email ? req.user.email.split('@')[0] : 'user-' + req.user.sub.slice(0, 6));

      // email may be null in token depending on your Cognito app client config — provide fallback
    ...
    if (existing) {
      ...
      return res.status(200).json({
        ...
        redeemed_at: existing.redeemedAt?.toISOString?.() || existing.redeemedAt || null,
      });
    }
